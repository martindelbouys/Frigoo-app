      ...
      <div style={{ position:'relative', background:'#fff', borderRadius:'28px 28px 0 0', maxHeight:'88%', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ flexShrink:0, padding:'12px 18px 0' }}>
          <div style={{ display:'flex', gap:8 }}>
            <input value={p.nrText} onChange={e=>p.setNrText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addIng()} placeholder="Ajouter un ingrédient…" autoFocus={searchingIng} style={{ flex:1, border:'1.5px solid #ECECEC', outline:'none', borderRadius:11, padding:'11px 13px', fontFamily:'inherit', fontSize:14, fontWeight:600 }} />
            {searchingIng
          ...
          <div className="fg-scroll" style={{ flex:1, overflowY:'auto', padding:'12px 18px' }}>
            <div style={{ background:'#fff', border:'1px solid #ECECEC', borderRadius:13, overflow:'hidden' }}>
              {p.nrSuggestions.map((s, i) => (
                <button key={i} onClick={()=>pickSugg(s)} style={{ width:'100%', display:'flex', alignItems:'center', gap:10, padding:'12px 14px', border:'none', borderBottom:'1px solid #F4F4F4', background:'#fff', cursor:'pointer', textAlign:'left', fontFamily:'inherit' }}>
                  <span style={{ fontSize:18, width:26, textAlign:'center' }}>{s.emoji}</span>
                  <span style={{ fontSize:14, fontWeight:700 }}>{s.name}</span>
                </button>
              ))}
              <button onClick={addIng} style={{ width:'100%', display:'flex', alignItems:'center', gap:10, padding:'12px 14px', border:'none', background:'#FFFBFA', cursor:'pointer', textAlign:'left', fontFamily:'inherit' }}>
